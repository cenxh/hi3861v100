...
void gpio_init(void)
{
    ...
    hi_io_set_func(IOT_GPIO_2, HI_IO_FUNC_GPIO_2_GPIO);
        ...
        if (len >= DTOF_DATA_LEN) 
        {
            if(uartReadBuff[0] == 0xAA && uartReadBuff[1] == 0x55 && uartReadBuff[132] == 0xFF) 
            {
                ...
                if(crc_data == *(unsigned int *)(&uartReadBuff[CRC_LEN])) 
                {
                    ...
                    if (r == 64)
                    { 
                        IoTGpioSetOutputVal(IOT_GPIO_2, IOT_GPIO_VALUE1);
                    ...
                    {
                        IoTGpioSetOutputVal(IOT_GPIO_2, IOT_GPIO_VALUE0);
